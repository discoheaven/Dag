...
void MyAlgo::initialize(const Dagger &dag, const System &sys, Scheduler &scheduler, int rank)
{
    Greedy g = Greedy();
    g.run(dag,sys,scheduler,rank);
...
void MyAlgo::run(const Dagger& dag, const System& sys, Scheduler& scheduler, int rank)
{
    ...
    for(int io = 0;io < grasp_iteration;++io) {
        ...
        for (int i = 0; i < 30; ++i) {
            auto critical_path = find_critical_path();
                      ...
                      const vector<shared_ptr<Processor>> &processors, Scheduler& scheduler)
{
    ...
    for(auto &new_processor : processors)
    {
        ...
        int child_min = static_cast<int>(temp.size());
...
void MyAlgo::reset(const Dagger &dag, const System &sys)
{
